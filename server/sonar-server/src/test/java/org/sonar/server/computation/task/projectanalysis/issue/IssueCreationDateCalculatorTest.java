...
public class IssueCreationDateCalculatorTest {
  ...
  @Test
  public void should_change_date_if_file_based_scm_is_available_and_rule_is_new() {
    previousAnalysisWas(2000L);
    currentAnalysisIs(3000L);
  ...
  @Test
  public void should_change_date_if_line_based_scm_is_available_and_rule_is_new() {
    previousAnalysisWas(2000L);
    currentAnalysisIs(3000L);
  ...
  private void withFileScm(long blame) {
  ...
  private void withLineScm(long blame) {
    ScmInfo scmInfo = mock(ScmInfo.class);
    Changeset changeset = Changeset.newChangesetBuilder().setDate(blame).setRevision("1").build();
    when(scmInfoRepository.getScmInfo(component))
      .thenReturn(Optional.of(scmInfo));
    when(issue.getLine()).thenReturn(LINE_NUMBER);
    when(scmInfo.getChangesetForLine(LINE_NUMBER)).thenReturn(changeset);
  }
